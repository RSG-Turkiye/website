 ...
 * `<path>/index.html` and Cloudflare Pages answers `/webinars/foo` with a 308
 * to `/webinars/foo/`. The canonical tags and the sitemap used the slash; the
 * links did not. 9,698 internal links across the built site -- every item in
 * the header and footer navigation, on every page -- cost a redirect before
 * they reached anything.
 *
...
test('no internal link points at a URL that redirects', { skip: !existsSync(DIST) && 'run `npm run build` first' }, () => {
  ...
  assert.ok(routes.size > 100, `expected the built site, found ${routes.size} pages`);
...
test('the built site is a directory-format build, which is why the above matters', () => {
  ...
  assert.ok(
    existsSync(join(DIST, 'about', 'index.html')),
    'expected dist/about/index.html -- a directory-format build',
  );
  assert.ok(
    !existsSync(join(DIST, 'about.html')),
    'found dist/about.html: this is now a file-format build, and internal ' +
