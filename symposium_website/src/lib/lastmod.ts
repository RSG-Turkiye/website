 ...
 *
 * The sitemap carried 43 URLs and no lastmod at all, which throws away the one
 * freshness signal a sitemap can give: a crawler has no way to tell the four
 * editions added this week from the ones untouched for a year.
 *
...
function sourceFor(pathname: string): string | null {
  ...
  // /editions/<year>/ is a dynamic route over the editions collection. The
  // Turkish body lives in its own file, so the Turkish page's date is that
  // file's -- the two are edited independently.
  if (rest.length === 2 && rest[0] === 'editions' && /^\d{4}$/.test(rest[1])) {
    const file = tr
      ? `src/content/editions-tr/${rest[1]}.md`
      : `src/content/editions/${rest[1]}.md`;
