---
number: 6
emoji: "🎬"
title: "RSG Türkiye, Uzmanları İzleyin"
subtitle: "Arşivimizdeki bu oturumlar başlangıç yapanlara uygundur. Gerçek araştırmacılar, gerçek problemler, iyi anlatılmış."
color: red
resources:
  - title: "Big Data in Life Sciences"
    description: "Modern biyolojinin hesaplamalı düşünce gerektiren ölçeklerde nasıl veri ürettiği. Alanın kapsamına harika bir giriş."
    tag: Webinar
    speaker: "Nikolay Oskolkov"
    year: 2022
    webinarSlug: big-data-in-life-sciences-nikolay-oskolkov
  - title: "Aspects of High-Throughput Molecular Data Analysis"
    description: "HT veri tipleri, ön işleme stratejileri ve yaygın analitik tuzakların geniş kapsamlı bir genel bakışı: verinin gerçek bir laboratuvarda nasıl aktığının mükemmel bir ilk bakışı."
    tag: Webinar
    webinarSlug: aspects-of-high-throughput-molecular-data-analysis
  - title: "Mapping Human Tissue Architecture, Spatial Transcriptomics"
    description: "Son teknoloji araştırmaya bir pencere. Her detayı anlamak için endişelenmeyin, sorulan biyolojik sorulara odaklanın."
    tag: Webinar
    speaker: "Dr. Ömer Ali Bayraktar"
    year: 2023
    webinarSlug: mapping-human-tissue-architecture-and-pathology-using-spatial-transcriptomics
  - title: "Early Detection of Skin Cancer with Deep Learning"
    description: "Gerçek bir klinik probleme uygulanan makine öğrenmesi. Hesaplamalı araçların tıbbı nasıl değiştirdiğinin net bir örneği."
    tag: Webinar
    webinarSlug: early-detection-skin-cancer-deep-learning-vidit-go
---
